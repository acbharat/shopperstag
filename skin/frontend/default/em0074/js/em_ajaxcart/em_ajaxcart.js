...
function setLocation(url) {
	if ($("enable_module").value == 1) window.location.href = url;
	else {
...
document.observe("dom:loaded", function () {
	if ($("enable_module").value == 1) return false;
		...
		$$('button.btn-cart').each(function (el) {
			...
			if (el.up('form#reorder-validate-detail')) {
				el.onclick = function () {
					...
					var param = $('reorder-validate-detail').serialize() + '&ajax_package_name=' + $('ajax_package_name').value + '&ajax_layout=' + $('ajax_layout').value + '&ajax_template=' + $('ajax_template').value + '&ajax_skin=' + $('ajax_skin').value;
